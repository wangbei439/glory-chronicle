...
func _build_scene() -> void:
        ...
        if equip_stats.get("max_hp_bonus", 0.0) > 0:
                warrior.max_hp += equip_stats["max_hp_bonus"]
        ...
        warrior.health_changed.connect(func(v): hud.update_player_hp(v, warrior.max_hp))
        ...
        # 操作提示
        var hint = Label.new()
        hint.text = "A/D:移动 W/Space:跳跃 J:轻攻 K:重攻 L:格挡 U:战吼 I:裂地斩 Tab:技能树 E:装备 R:重来 Esc:主菜单"
...
func _check_combat_collisions() -> void:
        ...
        if not warrior.is_attacking or warrior.attack_phase != 1:  # ACTIVE
                if warrior.attack_phase == 2:  # RECOVERY
...
func _check_lava_pool(delta: float) -> void:
        """地面熔岩池伤害"""
        if warrior.pos.y >= GROUND_Y - 5 and warrior.invincible_timer <= 0:
                if warrior.pos.x >= 250 and warrior.pos.x <= 390:
                        var dmg: float = 5.0 * (1.0 - skill_tree.get_defense_bonus()) * (1.0 - equipment.get_defense_bonus())
                        warrior.take_damage(dmg, Vector2(randf_range(-2, 2), -3))
...
func _process_earth_shatter(delta: float) -> void:
        if warrior.current_anim == "earth_shatter" and not get("earth_shatter_active"):
...
func _process_lava_projectiles(delta: float) -> void:
        for i in range(lava_projectiles.size() - 1, -1, -1):
                ...
                if proj["pos"].y >= GROUND_Y - 5:
                        var dist: float = abs(warrior.pos.x - proj["pos"].x)
                        if dist < 40 and warrior.invincible_timer <= 0:
                                var dmg: float = 10.0 * (1.0 - skill_tree.get_defense_bonus()) * (1.0 - equipment.get_defense_bonus())
                                warrior.take_damage(dmg, Vector2(randf_range(-3, 3), -4))
                                hud.spawn_damage_number(warrior.pos + Vector2(0, -40), dmg, false)
                                audio.play("hurt", 0.5)
                ...
                # 飞行中碰撞
                var dist: float = abs(warrior.pos.x - proj["pos"].x)
                var dist_y: float = abs(warrior.pos.y - 30 - proj["pos"].y)
                if dist < 25 and dist_y < 30 and warrior.invincible_timer <= 0:
...
func _run_demo() -> void:
        match frame_count:
                150:
                        warrior.vel.x = 220; warrior.facing = 1.0
                180:
                        warrior.do_attack("L"); audio.play("swing")
                270:
                        warrior.vel.x = 0
