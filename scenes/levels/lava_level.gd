...
func _build_scene() -> void:
        ...
        var hint = Label.new()
        hint.text = "A/D:移动 W/Space:跳跃 J:轻攻 K:重攻 L:格挡 U:战吼 I:裂地斩 Tab:技能树 E:装备 Esc:主菜单"
...
func _process_lava_pools(delta: float) -> void:
        ...
        var player_on_ground: bool = warrior.pos.y >= GROUND_Y - 5
        if not player_on_ground:
                return

        for pool in lava_pools:
                if warrior.pos.x >= pool["x"] and warrior.pos.x <= pool["x"] + pool["width"]:
        ...
        for geyser in geysers:
                if geyser["active"]:
                        ...
                        if not geyser["dealt_damage"]:
                                var dist_x: float = abs(warrior.pos.x - geyser["x"])
                                var dist_y: float = abs(warrior.pos.y - GROUND_Y)
                                if dist_x < 30 and dist_y < 40 and warrior.invincible_timer <= 0:
                                        var dmg: float = 12.0 * (1.0 - skill_tree.get_defense_bonus()) * (1.0 - equipment.get_defense_bonus())
                                        warrior.take_damage(dmg, Vector2(randf_range(-3, 3), -6))
                                        hud.spawn_damage_number(warrior.pos + Vector2(0, -40), dmg, false)
